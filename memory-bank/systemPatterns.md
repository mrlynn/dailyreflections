...
5. **Chatbot Crisis Detection**:
   - Requests run through keyword heuristics plus OpenAI moderation (`omni-moderation-latest`).
   - If flagged, API short-circuits with crisis templates (suicidal or relapse responses) and **never** queries RAG data.
   - Events stored in `crisis_logs` with hashed session ID, intent, template key, and trigger reasons—no raw message text.
   - Non-crisis responses pass a tone review step (`evaluateTone` + optional compassionate rewrite) to enforce warmth.

## Chatbot Feedback Intelligence

- **Collection Layout**:
  - `feedback`: raw thumbs up/down, tags, tone metadata, session hash.
  - `feedbackFlags`: per-response aggregates with severity scoring.
  - `feedbackDailyStats`: 30-day rollups for dashboard trend chart.
  - `feedbackRecommendations`: manual-review items for retrieval/prompt changes.
- **Scheduled Jobs** (Vercel Cron):
  - `scripts/feedback/analyzeFeedback.js` (daily): enrich feedback via OpenAI sentiment, update flags/stats, and create recommendation records. Never mutates retrieval weights automatically.
  - `scripts/feedback/cleanupFeedback.js` (weekly): prunes raw comments/content older than 90 days unless a response remains under review.
- **Manual Review Loop**:
  - Admins inspect `/admin/feedback` tabs (All, Flagged, Recommendations).
  - Recommendations track status (`pending`, `accepted`, `dismissed`) plus action history. Dismissed items return to `pending` if new negative signals arrive.
- **API Surface**:
  - `/api/admin/feedback/stats`: serves trend + flag summaries from rollups.
  - `/api/admin/feedback/flags`: exposes flagged responses.
  - `/api/admin/feedback/recommendations`: list/filter recommendation items.
  - `/api/admin/feedback/recommendations/[id]`: update status with optional note.
