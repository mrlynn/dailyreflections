/**
 * Daily Reflections SMS Sender Script
 *
 * This script is designed to be run as a cron job to send daily reflection SMS messages
 * to all eligible users. It respects each user's time zone and quiet hours preferences.
 *
 * Recommended cron schedule: Every hour
 *
 * The script will:
 * 1. Find all users with SMS enabled for daily reflections
 ...
 *
 * Usage:
 * node scripts/send-daily-reflections.js
 */
...
const { sendDailyReflectionToAllUsers } = require('../src/services/smsDeliveryService');

// Check if SMS feature is enabled
if (process.env.NEXT_PUBLIC_FEATURE_SMS !== 'true' || process.env.NEXT_PUBLIC_FEATURE_SMS_DAILY_REFLECTION !== 'true') {
  console.log('SMS feature or daily reflection SMS feature is not enabled. Exiting.');
  process.exit(0);
}

async function main() {
  console.log('Starting daily reflections SMS sender...');
  ...
  try {
    ...
    console.log(`Found ${eligibleUsers.length} eligible users for daily reflection SMS at this hour`);
    ...
    // Log the results
    console.log('Completed sending daily reflection SMS:');
    ...
    console.error('Error in daily reflections SMS sender:', error);
  } finally {
...
async function findEligibleUsers(db, currentHour) {
  // Find all users with SMS enabled for daily reflections
  // Only include users who have verified their phone number
  ...
  return allEligibleUsers.filter(user => {
    ...
    const preferredTime = user.preferences?.dailyReflectionTime || '07:00';
...
async function sendMessagesToEligibleUsers(eligibleUsers) {
  ...
  for (const user of eligibleUsers) {
    try {
      const result = await sendDailyReflectionToUser(user);
 ...
 */
async function sendDailyReflectionToUser(userPreference) {
  try {
    const userId = userPreference.userId.toString();

    // Import the service on demand to avoid circular dependencies
    const { sendDailyReflectionToUser } = require('../src/services/smsDeliveryService');

    // Send the reflection
    return await sendDailyReflectionToUser(userId);
  } catch (error) {
    console.error('Error in sendDailyReflectionToUser:', error);
    return {
      success: false,
...
main()
  .then(() => {
    console.log('Daily reflections SMS sender completed');
    process.exit(0);
  })
  .catch(error => {
    console.error('Fatal error in daily reflections SMS sender:', error);
    process.exit(1);
  });