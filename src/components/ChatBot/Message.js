...
export default function Message({
  ...
  const computedRole = message.role ?? role;
  const computedContent = message.content ?? content;
  const computedCitations = message.citations ?? citations;
  const createdAt = message.createdAt ?? null;
  const messageId = message.messageId ?? null;
  const [showCitations, setShowCitations] = useState(false);

  const isBot = computedRole === 'assistant';
  const helperId = useMemo(() => messageId || `message-${Math.random().toString(36).slice(2)}`, [messageId]);
    ...
    >
      ...
      >
        {isLoading ? (
          ...
          <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap' }} data-message-id={helperId}>
            {computedContent}
          </Typography>
        )}
      </Paper>
      ...
      {isBot && computedCitations && computedCitations.length > 0 && (
        <Box sx={{ mt: 1, width: '100%' }}>
          ...
          >
            <Typography variant="caption" color="text.secondary">
              {showCitations ? 'Hide Sources' : `Sources (${computedCitations.length})`}
            </Typography>
          ...
          <Collapse in={showCitations}>
            {computedCitations.map((citation, index) => (
              <Paper
                key={index}
                elevation={0}
                sx={{
      ...
      {isBot && !isLoading && (
        <ChatFeedback
          ...
          onSubmit={async (payload) => {
            ...
            await onFeedbackSubmit(message, payload);
          }}
