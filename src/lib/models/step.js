...
/**
 * Step Model - Represents a single step in the 12-step program
 */
export class Step {
  constructor({
    _id,
    ...
    this.number = number;
    this.title = title;
    this.text = text; // The short form text of the step
    this.longForm = longForm; // Optional longer version of the step text
    this.interpretations = interpretations; // Array of different interpretations
    this.keyPoints = keyPoints; // Main points to understand
    this.resources = resources; // Additional resources for this step
    this.reflectionReferences = reflectionReferences; // Daily reflection references
  }
}

/**
 * Steps Collection Operations
 */
export async function getSteps() {
  try {
    ...
    const steps = await collection.find({}).sort({ number: 1 }).toArray();
    return steps;
  } catch (error) {
 ...
 */
export async function getStepByNumber(number) {
  try {
    const client = await clientPromise;
    const db = client.db();
    const collection = db.collection('steps');
    const step = await collection.findOne({ number: parseInt(number, 10) });
    return step;
  } catch (error) {
...
/**
 * Get daily reflections related to a specific step
 */
export async function getStepReflections(stepNumber) {
  try {
    const client = await clientPromise;
    const db = client.db();

    // Find reflections that mention this step
    // This is a simple text search implementation. In a production environment,
    // you'd want a more robust search or pre-tagged reflections.
    const reflections = await db.collection('reflections')
      .find({
        $or: [
          { title: { $regex: `step ${stepNumber}`, $options: 'i' } },
          { quote: { $regex: `step ${stepNumber}`, $options: 'i' } },
          { comment: { $regex: `step ${stepNumber}`, $options: 'i' } }
        ]
      })
      .project({
        _id: 1,
    ...
    console.error(`Error fetching reflections for step ${stepNumber}:`, error);
    throw new Error('Failed to fetch step reflections');
  }
}

/**
 * Seed the database with the 12 steps if they don't already exist
 */
export async function seedSteps() {
  try {
    ...
    if (count > 0) {
      console.log('Steps already exist in database');
      return { success: true, message: 'Steps already exist' };
    }
    ...
    await collection.insertMany(stepsData);
    console.log('Successfully seeded 12 steps to database');
    return { success: true, message: 'Successfully seeded steps' };
  } catch (error) {
