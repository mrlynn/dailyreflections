...
import { sendDailyReflectionToUser, sendDailyReflectionToAllUsers } from '@/services/smsDeliveryService';

// POST /api/sms/send/reflection
// Endpoint to send daily reflection SMS
  ...
  try {
    ...
    if (result.success) {
      ...
      return NextResponse.json(
        { error: result.error || 'Failed to send daily reflection' },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error('Error in reflection SMS API:', error);
    return NextResponse.json(
