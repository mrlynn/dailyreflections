...
import { sendStep4CheckInToUser } from '@/services/smsDeliveryService';
import clientPromise from '@/lib/mongodb';
import { ObjectId } from 'mongodb';

// POST /api/sms/send/step4
// Endpoint to send Step 4 check-in SMS
export async function POST(request) {
  try {
    ...
    if (sendToAll) {
      ...
      // Find all users with SMS enabled for Step 4 check-ins
      ...
      const eligibleUsers = await db.collection('userSMSPreferences')
        .find({
          ...
          'preferences.step4CheckIn': true
        })
        .toArray();

      console.log(`Found ${eligibleUsers.length} eligible users for Step 4 SMS check-ins`);
      ...
      for (const user of eligibleUsers) {
        const result = await sendStep4CheckInToUser(user.userId.toString());
    ...
    const result = await sendStep4CheckInToUser(targetUserId);

    if (result.success) {
      return NextResponse.json(result);
    } else {
      return NextResponse.json(
        { error: result.error || 'Failed to send Step 4 check-in' },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error('Error in Step 4 check-in SMS API:', error);
    return NextResponse.json(
